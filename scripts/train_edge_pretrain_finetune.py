#!/usr/bin/env python
"""Pretrain a GNN encoder on benign data, then fine-tune for malicious edge detection.

Stage 1 (unsupervised): edge-type prediction on benign windows.
Stage 2 (supervised): binary edge classification on attack-labeled windows.
"""
...
def graph_to_data(path, node_vocab, edge_vocab, malicious_nodes=None):
    ...
    edge_label = torch.zeros((len(edges),), dtype=torch.long)
    edge_binary = torch.zeros((len(edges),), dtype=torch.float)
    ...
    for i, (src, dst, data) in enumerate(edges):
        ...
        edge_label[i] = edge_label_to_idx.get(label, edge_label_to_idx["UNK"])
    ...
    return Data(x=x, edge_index=edge_index, edge_attr=edge_attr, y=edge_binary, y_edge=edge_label)


class EdgeMLPBinary(nn.Module):
...
class EdgeMLPMulticlass(nn.Module):
...
def build_encoder(args, in_dim, edge_attr_dim):
...
def count_binary_labels(dataset):
    pos = 0
    total = 0
    for data in dataset:
...
def pretrain_encoder(args, encoder, decoder, train_set, val_set, edge_attr_dim):
    ...
    for epoch in range(1, args.pretrain_epochs + 1):
        ...
        for batch in train_loader:
            ...
            loss = criterion(logits, batch.y_edge)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for batch in val_loader:
                ...
                h = enc_out["h"]
                logits = decoder(h[batch.edge_index[0]], h[batch.edge_index[1]], batch.edge_attr)
                val_losses.append(criterion(logits, batch.y_edge).item())
        ...
        print(
            f"[pretrain] epoch {epoch:02d} train_loss={np.mean(losses):.6f} val_loss={val_loss:.6f}"
        )
...
def finetune_binary(args, encoder, decoder, train_set, val_set, test_set):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    encoder.to(device)
    decoder.to(device)
    pos, neg = count_binary_labels(train_set)
    if pos == 0:
        pos_weight = torch.tensor([1.0], device=device)
        print("Warning: no positive edges in finetune train set; using pos_weight=1.0")
    else:
    ...
    for epoch in range(1, args.finetune_epochs + 1):
        ...
        print(
            f"[finetune] epoch {epoch:02d} train_loss={np.mean(losses):.6f} val_loss={val_loss:.6f}"
        )
...
def split_finetune_sets(test_set, split_ratio, mal_indices, seed):
    n = len(test_set)
    indices = list(range(n))
    rng = random.Random(seed)
    rng.shuffle(indices)

    mal_set = set(mal_indices)
    mal_idx = [i for i in indices if i in mal_set]
    benign_idx = [i for i in indices if i not in mal_set]

    def split_list(lst):
        n_train = int(len(lst) * split_ratio[0])
        n_val = int(len(lst) * split_ratio[1])
        train = lst[:n_train]
        val = lst[n_train : n_train + n_val]
        test = lst[n_train + n_val :]
        return train, val, test

    mal_train, mal_val, mal_test = split_list(mal_idx)
    ben_train, ben_val, ben_test = split_list(benign_idx)

    train_idx = mal_train + ben_train
    val_idx = mal_val + ben_val
    test_idx = mal_test + ben_test
    rng.shuffle(train_idx)
    rng.shuffle(val_idx)
    rng.shuffle(test_idx)

    train_set = [test_set[i] for i in train_idx]
    val_set = [test_set[i] for i in val_idx]
    test_set = [test_set[i] for i in test_idx]
    return train_set, val_set, test_set, (len(mal_train), len(mal_val), len(mal_test))
...
def main():
    ...
    parser.add_argument("--output_dir", default="/home/pids/outputs/edge_pretrain_finetune")
    ...
    parser.add_argument("--finetune_split", default="0.6,0.2,0.2")
    ...
    split_ratio = [float(x) for x in args.finetune_split.split(",")]
    if len(split_ratio) != 3 or not math.isclose(sum(split_ratio), 1.0, rel_tol=1e-6):
        raise ValueError("--finetune_split must be three numbers summing to 1.0")
    ...
    mal_indices = sorted(test_idx_to_mal.keys())
    for seed in seeds:
        set_seed(seed)
        encoder = build_encoder(args, train_set[0].x.shape[1], edge_attr_dim)
        pre_decoder = EdgeMLPMulticlass(
            in_dim=args.out_dim * 2 + edge_attr_dim,
            hidden_dim=args.edge_hidden,
            out_dim=len(edge_vocab),
        )

        encoder = pretrain_encoder(args, encoder, pre_decoder, train_set, val_set, edge_attr_dim)

        finetune_train, finetune_val, finetune_test, mal_counts = split_finetune_sets(
            test_set, split_ratio, mal_indices, seed
        )
        pos_train, neg_train = count_binary_labels(finetune_train)
        pos_val, neg_val = count_binary_labels(finetune_val)
        pos_test, neg_test = count_binary_labels(finetune_test)
        print(
            "finetune window counts (malicious): "
            f"train={mal_counts[0]} val={mal_counts[1]} test={mal_counts[2]}"
        )
        print(
            "finetune edge counts (pos/neg): "
            f"train={pos_train}/{neg_train} val={pos_val}/{neg_val} test={pos_test}/{neg_test}"
        )
        bin_decoder = EdgeMLPBinary(in_dim=args.out_dim * 2 + edge_attr_dim, hidden_dim=args.edge_hidden)
        encoder, bin_decoder, metrics = finetune_binary(
            args, encoder, bin_decoder, finetune_train, finetune_val, finetune_test
        )

        run_dir = os.path.join(args.output_dir, f"seed_{seed}")
        os.makedirs(run_dir, exist_ok=True)
        torch.save(encoder.state_dict(), os.path.join(run_dir, "encoder.pt"))
        torch.save(bin_decoder.state_dict(), os.path.join(run_dir, "decoder.pt"))
